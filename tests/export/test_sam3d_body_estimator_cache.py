...
class Sam3DBodyEstimatorCacheTests(unittest.TestCase):
    ...
    def test_process_frames_records_cache_miss_frame_stem_without_print_spam(self):
        ...
        estimator.model.run_inference_batch.return_value = {
            "mhr": {
                "focal_length": torch.ones((1,), dtype=torch.float32),
                "pred_keypoints_3d": torch.zeros((1, 1, 3), dtype=torch.float32),
                "pred_keypoints_2d": torch.zeros((1, 1, 2), dtype=torch.float32),
                "pred_vertices": torch.zeros((1, 3, 3), dtype=torch.float32),
                "pred_cam_t": torch.zeros((1, 3), dtype=torch.float32),
                "pred_pose_raw": torch.zeros((1, 3), dtype=torch.float32),
                "global_rot": torch.zeros((1, 3), dtype=torch.float32),
                "body_pose": torch.zeros((1, 3), dtype=torch.float32),
                "hand": torch.zeros((1, 3), dtype=torch.float32),
                "scale": torch.zeros((1, 1), dtype=torch.float32),
                "shape": torch.zeros((1, 10), dtype=torch.float32),
                "face": torch.zeros((1, 10), dtype=torch.float32),
                "pred_joint_coords": torch.zeros((1, 1, 3), dtype=torch.float32),
                "joint_global_rots": torch.zeros((1, 1, 3, 3), dtype=torch.float32),
                "mhr_model_params": torch.zeros((1, 5), dtype=torch.float32),
            }
        }
        ...
        with patch(
            ...
            estimator.process_frames(
                img_list=["/tmp/folder/00000123.jpg"],
                bboxes=[np.array([[0.0, 0.0, 3.0, 3.0]], dtype=np.float32)],
